...
# Shapefile Paths - UPDATE THESE or set as environment variables
...
# Google Maps API Configuration - SET THIS AS ENVIRONMENT VARIABLE IN RAILWAY
...
# Server Configuration - Railway will provide PORT environment variable
...
<body>
    <div class="header">
        <h1>ðŸš´ Bike-Bus-Bike Route Planner <span class="speed-badge">@ GOOGLE MAPS BIKE TIMES</span></h1>
    ...
    <div class="container">
        ...
        <div id="sidebar">
            <div class="system-status">ðŸŸ¢ System Ready: Google Maps API + GTFS</div>
            ...
            <div class="click-instruction">
                ...
                ðŸŽ¨ Smart fallback for short bike segments
            </div>
            
            <!-- Facility Type Legend -->
            <div class="facility-legend">
                <h4>ðŸš´ Bike Facility Colors</h4>
            ...
            <button id="findRouteBtn" onclick="findTransitRoute()">ðŸ” Find Bike-Bus-Bike Routes</button>
            <button class="clear-btn" onclick="clearAll()">ðŸ—‘ï¸ Clear All</button>
    ...
    <script>
        ...
        function setOriginMarker(lat, lng) {{
            ...
            originMarker.bindPopup("ðŸš© Origin").openPopup();
        }}
        ...
        function setDestinationMarker(lat, lng) {{
            ...
            destinationMarker.bindPopup("ðŸŽ¯ Destination").openPopup();
        }}
        ...
        function showSpinner(show) {{
            ...
            document.getElementById('findRouteBtn').innerHTML = show ? 'â³ Finding Routes...' : 'ðŸ” Find Bike-Bus-Bike Routes';
        }}
        
        function showError(message) {{
            document.getElementById('results').innerHTML = '<div class="error"><strong>âš ï¸ Error:</strong> ' + message + '</div>';
        }}
        ...
        function displayEnhancedRoutes(data) {{
            if (!data.routes || data.routes.length === 0) {{
                document.getElementById('results').innerHTML = 
                    '<div class="info"><strong>â„¹ï¸ Info:</strong> No routes found</div>';
                return;
            }}
        ...
        function createDetailedTransitStep(step, stepIndex) {{
            ...
            if (isWalking) {{
                html += `
                    <div class="step-icon walk">ðŸš¶</div>
                    <div class="step-content">
                        <div class="step-title">Walk to ${{extractDestination(step.instruction)}}</div>
                        <div class="step-subtitle">${{step.duration_text}} â€¢ ${{(step.distance_meters / 1000).toFixed(2)}} km</div>
                    </div>`;
                    
            }} else if (isTransit) {{
                html += `
                    <div class="step-icon bus">ðŸšŒ</div>
                    <div class="step-content">
                        ...
                        <div class="transit-details">
                            ...
                            <div class="transit-info-line">
                                ...
                                <span>${{step.departure_stop_name}} â†’ ${{step.arrival_stop_name}}</span>
                            </div>
                            ...
                            ${{step.headsign ? `
                            <div class="transit-info-line">
                                <span style="font-size: 16px;">âž¡ï¸</span>
        ...
        function createRealtimeStatusDisplay(step) {{
            ...
            let html = `
                <div class="realtime-status ${{hasDelays ? 'delayed' : 'on-time'}}" style="margin-top: 8px;">
                    ðŸ”´ ${{hasDelays ? 'Delays Reported' : 'On Time'}}
                </div>`;
        ...
        function createEnhancedBikeStep(leg, legIndex) {{
            ...
            <div class="route-step">
                <div class="step-icon bike">ðŸš´</div>
                <div class="step-content">
                    <div class="step-title">${{leg.name}}</div>
                    <div class="step-subtitle">${{route.travel_time_formatted}} â€¢ ${{route.length_miles.toFixed(2)}} miles</div>
        ...
        function createEnhancedTransitStep(leg, legIndex) {{
            ...
            <div class="route-step">
                ...
                <div class="step-content">
                    <div class="step-title">${{leg.name}}</div>
                    <div class="step-subtitle">${{route.duration_text}} â€¢ ${{route.distance_miles.toFixed(2)}} miles</div>
                    ...
                    <div class="transit-details">
                        ...
                        <div class="transit-info-line">
                            <span style="font-size: 16px;">ðŸ“</span>
                            <span>${{route.start_stop?.name || 'Start Stop'}} â†’ ${{route.end_stop?.name || 'End Stop'}}</span>
                        </div>
        ...
        function createFacilityDistributionDisplay(facilityStats) {{
            ...
            let html = `
                <div class="facility-distribution">
                    <div class="facility-dist-title">ðŸ›£ï¸ BIKE INFRASTRUCTURE</div>
                ...
                if (leg.type === 'bike' && leg.route.segments && leg.route.segments.length > 0) {{
                    ...
                    routeLine.bindPopup(`
                        <div style="font-family:'Segoe UI',sans-serif;">
                            <h4 style="margin:0 0 10px 0;color:#3498db;">ðŸšŒ ${{leg.name}}</h4>
                    ...
                    routeLine.bindPopup(`
                        <div style="font-family:'Segoe UI',sans-serif;">
                            <h4 style="margin:0 0 10px 0;color:#27ae60;">ðŸš´ ${{leg.name}}</h4>
                ...
                if (transitLeg && transitLeg.route) {{
                    ...
                    if (transitRoute.start_stop) {{
                        ...
                        L.marker([transitRoute.start_stop.display_y, transitRoute.start_stop.display_x], {{icon: startIcon}})
                         .addTo(routeLayersGroup)
                         .bindPopup(`<strong>ðŸš Start Bus Stop</strong><br>${{transitRoute.start_stop.name}}`);
                    }}
                    ...
                    if (transitRoute.end_stop) {{
                        ...
                        L.marker([transitRoute.end_stop.display_y, transitRoute.end_stop.display_x], {{icon: endIcon}})
                         .addTo(routeLayersGroup)
                         .bindPopup(`<strong>ðŸš End Bus Stop</strong><br>${{transitRoute.end_stop.name}}`);
                    }}
                }}
            }}
...
def run_bike_bus_bike_google_server():
    ...
    try:
        ...
        try:
            ...
            print(f"\nBike-Bus-Bike Server running on port {PORT}")
            print(f"\nServer Features:")
            ...
            # Don't auto-open browser in production/Railway
